...
public class CustomerService {
    ...
    public List<CustomerInfoDTO> getCustomersWithConstructorExpression() {
        Query q = em.createQuery(
                "SELECT NEW control.CustomerInfoDTO(c.id, c.name, SUM(i.product.price))"
    ...
    public List<CustomerInfoDTO> getCustomersWithJooq() {
        ...
        List<CustomerInfoDTO> list = create.select(CUSTOMERS.ID, CUSTOMERS.NAME, sum(PRODUCTS.PRICE)).
                from(CUSTOMERS).
